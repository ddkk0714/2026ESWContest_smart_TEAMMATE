...
// 떨어진다.
//
// 통합 앱(`display/atlas/app`)의 `자세` 화면이다. 예전에는 `display/atlas/camtest`
// 라는 별도 앱이었는데, Pi 5 에 앱을 셋 깔아 두고 비교하던 단계가 끝나 하나로
// 합쳤다. 아래 `PostureScreen` 이 그 화면 전부이고, `PostureApp` 은 이 화면만
// 단독으로 띄워 보거나 위젯 테스트에서 쓰는 래퍼로 남긴다.
import 'dart:async';
import 'dart:io';
...
/// 자세 화면만 단독으로 띄우는 래퍼. 통합 앱은 `PostureScreen` 을 직접 쓴다.
