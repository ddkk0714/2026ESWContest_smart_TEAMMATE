...
/// am.com.atlas.app.camtest.service` 에는 systemd 의 시작/정지 줄만 찍히고 앱이
/// print 한 것은 사라진다. 그래서 UART 를 왜 못 열었는지 같은 것을 알아낼 방법이
/// 화면밖에 없었다. 파일 하나로 창구를 만든다.
///
///     ssh root@<보드> 'cat /data/share/usr/atlas/apps/com.atlas.app.camtest/camtest.log'
///
...
class Diag {
  ...
  static List<String> _defaultCandidates() {
    ...
    add(Platform.environment['ATLAS_APP_DIR'], 'camtest.log');
    add(File(Platform.resolvedExecutable).parent.path, 'camtest.log');
    add(env['HOME'], '.config/camtest/camtest.log');
    add('/tmp', 'camtest/camtest.log');
    return paths;
  }
