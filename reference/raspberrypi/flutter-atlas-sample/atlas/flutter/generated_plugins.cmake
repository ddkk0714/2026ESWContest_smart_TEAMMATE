...
set(APP_TYPE "third-party")  # Possible values: "system", "third-party"
  ...
  set(lib_glob_path "${CMAKE_SYSROOT}/${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_LIBDIR}/libvideo_player_atlas_plugin.so*")
  file(GLOB lib_files "${lib_glob_path}")
  message(STATUS "Found platforms plugin libraries: ${lib_files}")

  set(lib_path "${CMAKE_SYSROOT}/${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_LIBDIR}/libvideo_player_atlas_plugin.so.1")
  set(include_path "${CMAKE_CURRENT_SOURCE_DIR}/flutter/ephemeral/.plugin_symlinks/video_player_atlas/atlas/include/video_player_atlas/video_player_atlas_plugin.h")
  if(EXISTS "${lib_path}" AND EXISTS "${include_path}")
    message(STATUS "Using existing sysroot library and header for plugin: video_player_atlas")
    target_include_directories(${BINARY_NAME} PRIVATE flutter/ephemeral/.plugin_symlinks/video_player_atlas/atlas/include)
    target_link_libraries(${BINARY_NAME} PRIVATE video_player_atlas_plugin)
    if(NOT APP_TYPE STREQUAL "system")
      list(APPEND PLATFORM_PLUGIN_BUNDLED_LIBRARIES ${lib_files})
    endif()
  else()
    message(STATUS "Building plugin from source: video_player_atlas")
    add_subdirectory("flutter/ephemeral/.plugin_symlinks/video_player_atlas/atlas" "plugins/video_player_atlas")
    target_link_libraries(${BINARY_NAME} PRIVATE video_player_atlas_plugin)
    list(APPEND PLUGIN_BUNDLED_LIBRARIES
      $<TARGET_FILE:video_player_atlas_plugin>
      $<TARGET_SONAME_FILE:video_player_atlas_plugin>
      $<TARGET_LINKER_FILE:video_player_atlas_plugin>
    )
  endif()
