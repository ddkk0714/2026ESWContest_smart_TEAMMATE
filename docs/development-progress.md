...
|---|---|---|---|
| 보드 간 통신 | MQTT 우선, 데이터 무결성은 CRC로 판단 | envelope `1.0`·QoS·중복/유실/시간 규칙 명세 완료 | ESP32↔hub envelope 인코더·CRC 검증 구현 |
...
| Node-RED 역할 | **결정: 센서 대시보드·로깅·프로토타이핑. FSM 필수 경로에서는 제외** | UI 오류가 추론 안정성에 영향을 주지 않음 |
