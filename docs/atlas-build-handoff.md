...
### (해결됨) 터치 장애 — 2026-08-31 재현, 2026-09-14 화면 교체로 해소

> 새 디스플레이에서 터치가 정상 동작한다. 아래는 이전 화면의 기록이다.
...
1. 새 화면의 모델명·인터페이스·전원 경로를 `hardware.md`·`hardware-bringup.md` §1 에 기록한다.
2. 재부팅 자동 시작(DESKMATE 앱 + Pi 4 hub 서비스)을 등록하고 검증한다.
3. Pi 4 가 MQTT 를 발행하기 시작하면 `DESKMATE_MQTT_HOST` 빌드로 실제 Hub 연동을 검증한다(HTTP 는 fallback).
4. 상태 적응형 4화면(대기·저자극·상세·리포트)과 자동 실행 알림·되돌리기를 구현한다.
5. 실기 검증이 끝난 상태를 release `.ipk`로 다시 고정한다.
