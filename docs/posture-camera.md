...
| 🌀 **ToF 처럼 보기** | 영상 보기 안에서 전환. 카메라를 54x42 로 줄여 ToF 화면처럼 그린다 |
...
쪽에도 같은 격자를 붙였다 — 영상 보기 오른쪽 위에서 전환한다.

필터는 [`pico_esp32-cam_ftdi`](https://github.com/76EHwan/pico_esp32-cam_ftdi) 의
`tools/camera_source.py` 를 옮긴 것으로, 순서와 상수(`BG_PERIOD 4` · `BG_GUARD 24` ·
